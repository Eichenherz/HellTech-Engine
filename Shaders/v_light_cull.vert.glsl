...
void main()
{
	uint vertexId = uint( gl_VertexIndex & uint16_t( -1 ) );
	uint  lightId = uint( gl_VertexIndex >> 16 );

	vec3 pos = geom_ref( geomAddr ).pos[ vertexId ];
	light_data light = light_ref( lightsAddr ).lights[ lightId ];
