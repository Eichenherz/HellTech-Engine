...
inline void Win32WriteLastErr( LPTSTR lpsLineFile )
{
	...
	DWORD	bytesFormatted = FormatMessageA( FORMAT_MSG_FLAGS, nullptr, dwErr,
		0, msg, std::size( msg ), nullptr );

	if( 0 == bytesFormatted )
	{
		std::memset( msg, 0, std::size( msg ) );
		std::format_to_n( msg, std::size( msg ) - 1,
			"{} formated 0 bytes, exited with: {}", __func__, GetLastError() );
	}

	MessageBoxA( nullptr, ( LPCTSTR ) msg, TEXT( "Error" ),
